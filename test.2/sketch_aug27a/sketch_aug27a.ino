...
void loop() {
  if (Serial.available()) {                        // ตรวจสอบว่ามีข้อมูลส่งเข้ามาทาง Serial หรือไม่
    ...
    if (command == "15") {                         // ถ้าข้อมูลที่รับมาเป็น "15"
      ...
      myservo.write(15);  
      digitalWrite(2, HIGH);                          // หมุน Servo ไปที่มุม 15 องศา
      delay(500);  
      digitalWrite(2, LOW);                                  // กระพริบ LED แบบเร็ว
    } 
    ...
    delay(500);                                   // หน่วงเวลา 1 วินาทีเพื่อไม่ให้ loop ทำงานเร็วเกินไป
  }
}
...
void treeLED(){                                     // ฟังก์ชันสำหรับทำให้ LED กระพริบช้า
  for(int i = 0; i<4; i++){                        // ทำซ้ำ 5 รอบ
    digitalWrite(2, HIGH);                         // เปิด LED
    delay(3000);                                   // หน่วงเวลา 2 วินาที
    digitalWrite(2, LOW);                          // ปิด LED
    delay(3000);                                   // หน่วงเวลา 2 วินาที
  }
}